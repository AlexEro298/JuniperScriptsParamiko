...
def compare_value_is_not_null(show_config_bgp_group,group_name):
    if show_config_bgp_group==(''):
...
for name_group_v4 in group_ipv4:
    ...
    if compare_value_is_not_null(show_config_bgp_group,name_group_v4):
        print(f'Name group - {name_group_v4}:')
        for ip_neighboor in neighbor_ipv4_re.findall(show_config_bgp_group):
            print(f'IP neighbor: {ip_neighboor}')
            show_route_receive_protocol_bgp = network_connect.send_command(
                f'show route receive-protocol bgp {ip_neighboor}')
...
for name_group_v6 in group_ipv6:
    ...
    if compare_value_is_not_null(show_config_bgp_group,name_group_v6):
        print(f'Name group - {name_group_v6}:')
        for ip_neighboor in neighbor_ipv4_re.findall(show_config_bgp_group):
            print(f'IP neighbor: {ip_neighboor}')
            show_route_receive_protocol_bgp = network_connect.send_command(
                f'show route receive-protocol bgp {ip_neighboor}')
